...
export default class Database {
        ...
        .then(() => {
          ...
          SQLite.openDatabase(
            ...
            .then((DB) => {
              ...
              db.executeSql('SELECT 1 FROM steps LIMIT 1')
                .then(() => {
                ...
                .catch((error) => {
                  ...
                  db.transaction((tx) => {
                    tx.executeSql(
                      'CREATE TABLE IF NOT EXISTS [steps] ([id] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, [pId] NVARCHAR(50) NULL,[pName] NVARCHAR(255) NULL,[pDescription] NVARCHAR(255) NULL,[pPrice] NVARCHAR(255) NULL,[pOnePrice] NVARCHAR(255) NULL,[pQty] INTEGER NOT NULL,[pImage] NVARCHAR(1000) NULL,[pExtra] NVARCHAR(500) NULL,[pSize] NVARCHAR(255) NULL, [pStatus] INTEGER NOT NULL)',
                    );
                    tx.executeSql(
  ...
  addtocart(db, data) {
    ...
    return new Promise((resolve) => {
      db.transaction((tx) => {
        ...
        tx.executeSql(
          'INSERT OR IGNORE  INTO steps (pId,pName,pDescription,pPrice,pOnePrice,pQty,pImage,pExtra,pSize,pStatus) VALUES ( ?,?,?,?,?,?,?,?,?,?)',
          [
            data.p_id,
            data.p_name,
            data.p_description,
  ...
  getMinusValue(db,cid) {
    return new Promise((resolve) => {
      let cart_dataminus;
      db.transaction((tx) => {
        tx.executeSql('SELECT pQty FROM steps c WHERE c.cId=?', [cid]).then(
          ([tx, results]) => {
            var len = results.rows.length;
  ...
  listCartData(db) {
    return new Promise((resolve) => {
      ...
      let cart_topins = [];
      db.transaction((tx) => {
        tx.executeSql('SELECT * FROM steps c ORDER BY c.cId DESC', []).then(
          ([tx, results]) => {
            var len = results.rows.length;
            let cart_topins22 = [];
            for (let i = 0; i < len; i++) {
  ...
  deleteItem(db, id) {
    return new Promise((resolve) => {
      db.transaction((tx) => {
        tx.executeSql('DELETE FROM steps WHERE cId = ?', [id]).then(
          ([tx, results]) => {
            resolve(results);
          },
        );
  ...
  deleteCartData(db) {
    return new Promise((resolve) => {
      db.transaction((tx) => {
        tx.executeSql('DELETE FROM steps', []).then(([tx, results]) => {
          resolve(results);
        });
  ...
  cartCont(db) {
    ...
    return new Promise((resolve) => {
      let cart_count;
      db.transaction((tx) => {
        tx.executeSql('SELECT COUNT(cId) AS cartcount FROM steps', []).then(
          ([tx, results]) => {
            var len = results.rows.length;
            for (let i = 0; i < len; i++) {
              let row = results.rows.item(i);
  ...
  updateCart(db, qty, newPrice, id) {
    return new Promise((resolve) => {
      db.transaction((tx) => {
        tx.executeSql(
          'UPDATE steps SET pQty =pQty + ?,pPrice=?   WHERE pId = ?',
          [qty, newPrice, id],
        ).then(([tx, results]) => {
          resolve(results);
        });
  ...
  addItemQty(db, data) {
    return new Promise((resolve) => {
      db.transaction((tx) => {
        tx.executeSql(
          'UPDATE steps SET pQty =pQty + ?,pPrice=?   WHERE cId = ? ',
  ...
  getItemById(db, id) {
    return new Promise((resolve) => {
      const cart_item = [];
      db.transaction((tx) => {
        tx.executeSql('SELECT cId,pId,pPrice,pQty FROM steps WHERE pId = ? ', [
          id,
        ]).then(([tx, results]) => {
          var len = results.rows.length;
          for (let i = 0; i < len; i++) {
